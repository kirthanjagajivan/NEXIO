...
export function TrainerTaskAnalytics() {
  ...
  const STATUS_CONFIG = {
    pending:        { label: t.pending,               icon: <Clock size={13} />,          cls: 'bg-slate-700 text-slate-300 border-slate-600' },
    submitted:      { label: t.stat_under_review,     icon: <Clock size={13} />,          cls: 'bg-amber-500/20 text-amber-300 border-amber-500/30' },
    approved:       { label: t.status_approved,       icon: <CheckCircle size={13} />,    cls: 'bg-green-500/20 text-green-300 border-green-500/30' },
    rejected:       { label: t.status_rejected,       icon: <XCircle size={13} />,        cls: 'bg-red-500/20 text-red-300 border-red-500/30' },
    needs_revision: { label: t.status_needs_revision, icon: <RotateCcw size={13} />,      cls: 'bg-blue-500/20 text-blue-300 border-blue-500/30' },
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">{t.task_analytics_title}</h2>
            <p className="text-sm text-slate-400">{t.task_analytics_desc}</p>
          </div>
        </div>
        <div className="flex flex-col items-center justify-center py-16 gap-4 bg-[#1E293B] rounded-xl border border-slate-700/60">
          ...
          <div className="text-center">
            <p className="font-semibold text-slate-300">{t.no_tasks_submitted}</p>
            <p className="text-sm text-slate-500 mt-1">{t.no_tasks_submitted_desc}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          onClick={load}
          className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-slate-800 rounded-lg border border-slate-700 transition-colors shrink-0"
        >
          <RefreshCw size={13} />
          {t.refresh_data}
        </button>
        ...
        <div className="bg-[#1E293B] rounded-xl border border-slate-700/60 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-white text-sm">{t.practical_task_completion}</span>
            </div>
            <span className="text-2xl font-bold text-amber-400">{completionPct}%</span>
          </div>
          ...
          <p className="text-xs text-slate-500 mt-2">
            <span className="font-semibold text-slate-300">{passedPerf}</span> {t.of}{' '}
            <span className="font-semibold text-slate-300">{perfRecords.length}</span> {t.tasks_passed_of}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: t.stat_submitted, value: submissions.length, color: 'text-white' },
          { label: t.stat_approved, value: approved, color: 'text-green-400' },
          { label: t.stat_under_review, value: underReview + needsRevision, color: 'text-amber-400' },
          { label: t.stat_avg_score, value: avgScore !== null ? `${avgScore}/100` : '—', color: avgScore !== null && avgScore >= 60 ? 'text-blue-400' : 'text-white' },
        ].map((c) => (
      ...
      <div className="bg-[#1E293B] rounded-xl border border-slate-700/60 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-white text-sm">{t.status_breakdown}</h3>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: t.status_approved,       count: approved,      cls: 'bg-green-500/20 border-green-500/30 text-green-300',  icon: <CheckCircle size={14} /> },
            { label: t.status_rejected,       count: rejected,      cls: 'bg-red-500/20 border-red-500/30 text-red-300',        icon: <XCircle size={14} /> },
            { label: t.status_under_review,   count: underReview,   cls: 'bg-amber-500/20 border-amber-500/30 text-amber-300',  icon: <Clock size={14} /> },
            { label: t.status_needs_revision, count: needsRevision, cls: 'bg-blue-500/20 border-blue-500/30 text-blue-300',     icon: <RotateCcw size={14} /> },
          ].map((s) => (
      ...
      <div className="bg-[#1E293B] rounded-xl border border-slate-700/60 overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-700/60 bg-slate-900/50">
          ...
          <h3 className="font-semibold text-white text-sm">{t.submission_history}</h3>
        </div>
            ...
            return (
              <div key={sub.id}>
                <button
                  ...
                  className="w-full px-5 py-4 flex items-center gap-3 hover:bg-slate-800/50 transition-colors text-left"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-white text-sm truncate">{sub.task_title}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {(hasFeedback || hasHistory) && (isOpen ? <ChevronUp size={15} className="text-slate-500" /> : <ChevronDown size={15} className="text-slate-500" />)}
                  </div>
                </button>

                {isOpen && (hasFeedback || hasScore || hasHistory) && (
                  <div className="px-5 pb-5 pt-1 bg-slate-900/30 border-t border-slate-700/60 space-y-3">
                    {hasFeedback && (
                      <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <MessageSquare size={13} className="text-slate-500" />
                          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide">{t.trainer_feedback_label}</span>
                        </div>
                        <p className="text-sm text-slate-200 leading-relaxed">{sub.feedback}</p>
                        {sub.reviewed_at && (
                          <p className="text-xs text-slate-500 mt-2">{t.reviewed_on} {formatDate(sub.reviewed_at)}</p>
                        )}
                      </div>
                    )}
                      ...
                      <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide">{t.score_history}</span>
                          </div>
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${perf.passed ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
                        ...
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <span className="text-sm font-bold text-white shrink-0">{perf.score}/100</span>
                        </div>
                        {perf.score_history.length > 1 && (
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-xs text-slate-500 shrink-0">{t.attempts}:</span>
                              ...
                              return (
                                <div key={i} className="flex items-center gap-0.5">
                                  ...
                                  <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full border ${
                                    s >= 80 ? 'text-green-300 bg-green-500/20 border-green-500/30' :
                                    s >= 60 ? 'text-blue-300 bg-blue-500/20 border-blue-500/30' :
                                    s >= 40 ? 'text-amber-300 bg-amber-500/20 border-amber-500/30' :
                                    'text-red-300 bg-red-500/20 border-red-500/30'
                                  } ${i === perf.score_history.length - 1 ? 'ring-1 ring-offset-1 ring-slate-700' : ''}`}>
                                    {s}/100
                                  </span>
                                </div>
                              );
                    ...
                    {!hasFeedback && sub.status === 'submitted' && (
                      <div className="flex items-center gap-2 text-xs text-amber-300 bg-amber-500/20 border border-amber-500/30 rounded-lg px-4 py-3">
                        <AlertTriangle size={13} />
                        {t.awaiting_review_msg}
                      </div>
                    )}
                    {!hasFeedback && sub.status === 'needs_revision' && (
                      <div className="flex items-center gap-2 text-xs text-blue-300 bg-blue-500/20 border border-blue-500/30 rounded-lg px-4 py-3">
                        <RotateCcw size={13} />
      ...
      {perfRecords.length > 0 && (
        <div className="bg-[#1E293B] rounded-xl border border-slate-700/60 overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-700/60 bg-slate-900/50">
            ...
            <h3 className="font-semibold text-white text-sm">{t.reviewed_task_scores}</h3>
          </div>
          <div className="divide-y divide-slate-700/50">
            {perfRecords.map((p) => (
              <div key={p.id} className="px-5 py-4">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${p.passed ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
                      {p.passed ? <CheckCircle size={14} className="text-green-400" /> : <XCircle size={14} className="text-red-400" />}
                    </div>
                    <div className="min-w-0">
                      <p className="font-semibold text-white text-sm truncate">{p.task_title}</p>
                ...
                <div className="mt-2.5 flex items-center gap-3">
                  ...
                  <span className={`text-xs font-bold shrink-0 ${p.passed ? 'text-green-400' : 'text-red-400'}`}>
                    {p.passed ? t.pass_label : t.fail_label}
                  </span>
                </div>
                {p.feedback && (
                  <div className="mt-2.5 flex items-start gap-2 text-xs text-slate-300 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5">
