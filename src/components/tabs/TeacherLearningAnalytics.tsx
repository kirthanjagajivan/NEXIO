...
function ScoreBar({ score, total }: { score: number; total: number }) {
  ...
  return (
    <div className="flex items-center gap-3 flex-1">
      ...
      <span className="text-sm font-bold text-gray-700 w-14 text-right shrink-0">{score}/{total}</span>
    </div>
  );
}
...
export function TeacherLearningAnalytics() {
  ...
  const TREND_CONFIG: Record<Trend, { label: string; desc: string; icon: React.ReactNode; bg: string; border: string; lc: string; dc: string; rec: string }> = {
    excellent: {
      ...
      bg: 'bg-amber-50', border: 'border-amber-200', lc: 'text-amber-800', dc: 'text-amber-600',
    ...
    improving: {
      ...
      bg: 'bg-blue-50', border: 'border-blue-200', lc: 'text-blue-800', dc: 'text-blue-600',
    ...
    low: {
      ...
      bg: 'bg-red-50', border: 'border-red-200', lc: 'text-red-800', dc: 'text-red-600',
    ...
    neutral: {
      ...
      bg: 'bg-gray-50', border: 'border-gray-200', lc: 'text-gray-800', dc: 'text-gray-500',
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900">Teacher Learning Analytics</h2>
            <p className="text-sm text-gray-500">Quiz scores, lesson progress, and chapter breakdown</p>
          </div>
        </div>
        <div className="flex flex-col items-center justify-center py-16 gap-4 bg-white rounded-xl border border-gray-200">
          ...
          <div className="text-center">
            <p className="font-semibold text-gray-700">{t.noPerformanceData}</p>
            <p className="text-sm text-gray-400 mt-1">{t.complete_lesson_prompt}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={load}
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-gray-100 rounded-lg border border-gray-200 transition-colors"
          >
            <RefreshCw size={13} />
            Refresh
          </button>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 flex items-start gap-1.5">
              ...
              <p className="text-xs text-gray-600 italic">{cfg.rec}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div className="flex justify-between mt-2 text-xs text-gray-400">
          <span><span className="font-semibold text-gray-600">{records.length}</span> of <span className="font-semibold text-gray-600">{totalTopics}</span> {t.lessons_completed_of}</span>
          {records.length === totalTopics
            ? <span className="text-green-600 font-semibold bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">{t.all_lessons_done}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Topics Attempted', value: records.length, color: 'text-gray-900' },
          { label: t.passed_stat, value: passed, sub: `${failed} ${t.failed_label}`, color: 'text-green-600' },
          { label: t.avg_score, value: `${avgScore}/50`, sub: `${avgPct}%`, color: avgPct >= 60 ? 'text-blue-600' : 'text-amber-600' },
          { label: t.total_attempts, value: totalAttempts, sub: t.across_all_lessons, color: 'text-gray-900' },
        ].map((c) => (
          <div key={c.label} className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col gap-1">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider leading-tight">{c.label}</p>
      ...
      {weakRecords.length > 0 && (
        <div className="bg-white rounded-xl border border-amber-200 overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-3.5 bg-amber-50 border-b border-amber-200">
            ...
            <h3 className="font-semibold text-amber-900 text-sm">Weak Topics — Needs Review</h3>
          </div>
              ...
              return (
                <div key={r.topicId} className="px-5 py-3 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate">{r.lessonName}</p>
                    <p className="text-xs text-gray-400">{r.attempts} attempt{r.attempts !== 1 ? 's' : ''}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
      ...
      {chapterGroups.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 px-1">
            ...
            <h3 className="font-semibold text-gray-800 text-sm">Results by Chapter</h3>
          </div>
          {chapterGroups.map((group) => {
            ...
            const gAvg = gTotal > 0
              ...
              : 0;
            return (
              <div key={group.chapterTitle} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <p className="font-semibold text-gray-900 text-sm">{group.chapterTitle}</p>
                    <p className="text-xs text-gray-400">{gTotal} topic{gTotal !== 1 ? 's' : ''} attempted</p>
                  </div>
                  <div className="flex items-center gap-4 shrink-0">
                    ...
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="font-bold text-sm text-gray-800">{gPassed}/{gTotal}</p>
                    </div>
                        ...
                        <div key={record.topicId} className="px-5 py-4">
                          <div className="flex items-start justify-between gap-3 flex-wrap">
                            <div className="flex items-center gap-2.5 min-w-0">
                              <div className={`shrink-0 w-7 h-7 rounded-full flex items-center justify-center ${record.passed ? 'bg-green-100' : 'bg-red-100'}`}>
                                {record.passed
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${record.passed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
      ...
      {ungrouped.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-100 bg-gray-50">
            ...
            <h3 className="font-semibold text-gray-800 text-sm">Other Lessons</h3>
          </div>
          <div className="divide-y divide-gray-100">
            {ungrouped.map((record) => {
              ...
              return (
                <div key={record.topicId} className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${record.passed ? 'bg-green-100' : 'bg-red-100'}`}>
                      {record.passed ? <CheckCircle size={14} className="text-green-600" /> : <XCircle size={14} className="text-red-500" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-900 text-sm truncate">{record.lessonName}</p>
                    </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-800 text-sm">Quiz Performance Summary</h3>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-bold text-green-600">{passed}</p>
            <p className="text-xs text-gray-400 mt-0.5">Topics Passed</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-red-500">{failed}</p>
            <p className="text-xs text-gray-400 mt-0.5">Topics Failed</p>
          </div>
          <div>
            <p className={`text-2xl font-bold ${avgPct >= 60 ? 'text-blue-600' : 'text-amber-600'}`}>{avgPct}%</p>
