...
import { useTraineePerformance, type TraineePerformanceData } from '../../hooks/useTraineePerformance';
...
export function PerformanceMonitorTab({ traineeIds, title, subtitle }: PerformanceMonitorTabProps) {
  const { t } = useLanguage();
  const [expandedTrainee, setExpandedTrainee] = useState<string | null>(null);
  const { traineeData, loading, error, refetch } = useTraineePerformance(traineeIds);
...
function TraineePerformanceCard({
  data,
  expanded,
  onToggle,
}: {
