...
import { usePracticalPerformance, type TraineePracticalData } from '../../hooks/usePracticalPerformance';

interface PracticalPerformanceTabProps {
  traineeIds?: string[];
  title: string;
  subtitle: string;
}

export function PracticalPerformanceTab({ traineeIds, title, subtitle }: PracticalPerformanceTabProps) {
  const { t } = useLanguage();
  const [expandedTrainee, setExpandedTrainee] = useState<string | null>(null);
  const { traineeData, loading, error, refetch } = usePracticalPerformance(traineeIds);

  const stats = traineeData.reduce(
    (acc, td) => ({
      totalTrainees: acc.totalTrainees + 1,
      totalTasks: acc.totalTasks + td.stats.totalTasks,
      passedTasks: acc.passedTasks + td.stats.passedTasks,
      avgScoreSum: acc.avgScoreSum + td.stats.avgScore,
    }),
    { totalTrainees: 0, totalTasks: 0, passedTasks: 0, avgScoreSum: 0 }
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold text-amber-300">{t.practical_training || 'Practical Training'}</p>
          <p className="text-sm text-amber-400">{t.practical_training_desc || 'Workplace tasks, assignments, and company training content'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            <ClipboardCheck className="text-green-400" size={18} />
            <span className="text-sm font-medium text-green-300">{t.passed_tasks || 'Passed Tasks'}</span>
          </div>
          <p className="text-3xl font-bold text-green-300">{stats.passedTasks}</p>
        </div>
        ...
        <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-purple-300">{t.practical_progress || 'Practical Progress'}</span>
          </div>
          <p className="text-3xl font-bold text-purple-300">
            {stats.totalTasks > 0
              ? Math.round((stats.passedTasks / stats.totalTasks) * 100)
              : 0}
            %
          </p>
        </div>
      ...
      <div className="bg-[#1E293B] rounded-xl border border-slate-700/60 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-700/60 bg-slate-900/50">
          <h2 className="font-semibold text-white text-sm">{t.practical_progress || 'Practical Progress'}</h2>
        </div>
...
function PracticalTraineeCard({
  data,
  expanded,
  onToggle,
}: {
  data: TraineePracticalData;
  expanded: boolean;
  onToggle: () => void;
}) {
  const { t } = useLanguage();
    ...
    <div className="hover:bg-slate-700/30 transition-colors">
      ...
      >
        <div className="flex items-center gap-4 flex-1">
          <div className="w-11 h-11 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-right">
              <p className="text-xs text-slate-400">{t.tasks_completed || 'Tasks Completed'}</p>
              <p className="font-bold text-white">
                {stats.passedTasks}/{stats.totalTasks}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
              <h4 className="font-semibold text-white text-sm mb-3 flex items-center gap-2">
                ...
                {t.weak_tasks || 'Weak Tasks'}
              </h4>
              {stats.weakTasks.length === 0 ? (
                <p className="text-xs text-green-400">{t.all_tasks_passed || 'All tasks passed!'}</p>
              ) : (
                <div className="space-y-2">
                  {stats.weakTasks.map((task, i) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      <span className="text-slate-300 truncate max-w-[180px]">{task.taskTitle}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-amber-400 font-medium">{task.avgScore}%</span>
                        <span className="text-slate-500">({task.attempts}x)</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <div className="w-full bg-slate-700 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${stats.totalTasks > 0 ? (stats.passedTasks / stats.totalTasks) * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
